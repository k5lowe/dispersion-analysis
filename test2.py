"""
Python script to automate historical data collection using Open-Meteo's API. This version is a first iteration
operating through command line. Future versions likely include a UI for ease-of-use.
Author: 	Kushini Lowe with contributions from Luca Scavone
Created: 	Fall 2025
Subsystem: 	Flight Dynamics
Project:	Dispersion Analysis
"""
...
def lon_coords_input():
	...
	while True:
		lon_input = input("Launch longitude (hit enter for default):")
		try:
			if lon_input.strip() == "":
...
for i in date_ranges:
	...
	hourly_wind_speed_100hPa 		= hourly.Variables(16).ValuesAsNumpy()
	hourly_wind_speed_70hPa 		= hourly.Variables(17).ValuesAsNumpy()
	...
	wind_speeds.append(hourly_wind_speed_1000hPa)
	wind_speeds.append(hourly_wind_speed_975hPa)
	wind_speeds.append(hourly_wind_speed_950hPa)
	wind_speeds.append(hourly_wind_speed_925hPa)
	wind_speeds.append(hourly_wind_speed_900hPa)
	wind_speeds.append(hourly_wind_speed_850hPa)
	wind_speeds.append(hourly_wind_speed_800hPa)
	wind_speeds.append(hourly_wind_speed_700hPa)
	wind_speeds.append(hourly_wind_speed_600hPa)
	wind_speeds.append(hourly_wind_speed_500hPa)
	wind_speeds.append(hourly_wind_speed_400hPa)
	wind_speeds.append(hourly_wind_speed_300hPa)
	wind_speeds.append(hourly_wind_speed_250hPa)
	wind_speeds.append(hourly_wind_speed_200hPa)
	wind_speeds.append(hourly_wind_speed_150hPa)
	wind_speeds.append(hourly_wind_speed_100hPa)
	wind_speeds.append(hourly_wind_speed_70hPa)
	wind_speeds.append(hourly_wind_speed_50hPa)
	wind_speeds.append(hourly_wind_speed_30hPa)
	wind_directions.append(hourly_wind_direction_1000hPa)
	wind_directions.append(hourly_wind_direction_975hPa)
	wind_directions.append(hourly_wind_direction_950hPa)
	wind_directions.append(hourly_wind_direction_925hPa)
	wind_directions.append(hourly_wind_direction_900hPa)
	wind_directions.append(hourly_wind_direction_850hPa)
	wind_directions.append(hourly_wind_direction_800hPa)
	wind_directions.append(hourly_wind_direction_700hPa)
	wind_directions.append(hourly_wind_direction_600hPa)
	wind_directions.append(hourly_wind_direction_500hPa)
	wind_directions.append(hourly_wind_direction_400hPa)
	wind_directions.append(hourly_wind_direction_300hPa)
	wind_directions.append(hourly_wind_direction_250hPa)
	wind_directions.append(hourly_wind_direction_200hPa)
	wind_directions.append(hourly_wind_direction_150hPa)
	wind_directions.append(hourly_wind_direction_100hPa)
	wind_directions.append(hourly_wind_direction_70hPa)
	wind_directions.append(hourly_wind_direction_50hPa)
	wind_directions.append(hourly_wind_direction_30hPa)

	# ========================================================================
	# Make raw weather data .csv file
	# ========================================================================
	...
	# ========================================================================
	# Make simulation parameters .csv file
	# ========================================================================
	sim_parameters = dict(date=pd.date_range(
		start=pd.to_datetime(hourly.Time(), unit="s", utc=True),
		end=pd.to_datetime(hourly.TimeEnd(), unit="s", utc=True),
		freq=pd.Timedelta(seconds=hourly.Interval()),
		inclusive="left"
	...
	print("THis is sim_params\n", sim_parameters)

	sim_parameters["date"] 							= sim_parameters["date"].strftime("%Y-%m-%d %H:%M")
	sim_parameters["date"] 							= pd.to_datetime(sim_parameters["date"])
	sim_parameters["temperature"] 					= hourly_temperature_2m
	sim_parameters["pressure"] 						= hourly_pressure_msl
	# print("THIS IS PRESSURE OR HOURLY_PRESSURE_MSL\n", sim_parameters["pressure"])

	csv_size = len(sim_parameters["date"])
	for altitude, wind_speed, wind_direction in zip(altitudes, wind_speeds, wind_directions):
		...
		sim_parameters[f"{altitude}"] 				= wind_speed
		sim_parameters[f"stdev [{altitude}]"] 		= np.zeros(csv_size)
		sim_parameters[f"direction [{altitude}]"] 	= wind_direction

	sim_parameters["cloud_cover_low"] 				= hourly_cloud_cover_low
	sim_parameters["is_day"] 						= hourly_is_day

	sim_parameters_dataframe = pd.DataFrame(data=sim_parameters)

	# ========================================================================
	# Filter data and save as .csv files
	# ========================================================================
	start_time 	= pd.to_datetime("11:00").time()
	end_time 	= pd.to_datetime("19:00").time()

	# Ask for base file name (no extension required)
	base_name 				= input("Enter raw weather data file name \n RECOMMENDED [date range]_open-meteo \n>> ").strip()
	sim_params_name 		= input("Enter sim parameters file name \n RECOMMENDED sim_parameters_historical_[date range] \n>> ").strip()
	raw_weather_directory 	= "Raw Weather Data/"
	sim_params_directory 	= "Processed Weather Data/"

	# Build filenames
	full_file_name 			= raw_weather_directory + base_name + "_full-data.csv"
	filtered_file_name 		= raw_weather_directory + base_name + "_filtered.csv"

	full_sim_params_name 	= sim_params_directory + sim_params_name + "_full-data.csv"
	filtered_sim_params_name = sim_params_directory + sim_params_name + "_filtered.csv"

	# Save full hourly data
	hourly_dataframe.to_csv(full_file_name, index=False)
	print(f"Full hourly data saved to {full_file_name}")

	sim_parameters_dataframe.to_csv(full_sim_params_name, index=False)
	print(f"Full hourly data saved to {full_sim_params_name}")
	...
	filtered_sim_params_dataframe = sim_parameters_dataframe[
		(sim_parameters_dataframe['is_day'] == 1) &
		(sim_parameters_dataframe['cloud_cover_low'] <= 50) &
		(sim_parameters_dataframe['date'].dt.time >= start_time) &
		(sim_parameters_dataframe['date'].dt.time <= end_time)
		]

	del filtered_sim_params_dataframe["cloud_cover_low"]
	del filtered_sim_params_dataframe["is_day"]

	print("\nFiltered hourly data\n", filtered_sim_params_dataframe)

	# Save filtered hourly data
	filtered_hourly_dataframe.to_csv(filtered_file_name, index=False)
	print(f"Filtered hourly data saved to {filtered_file_name}")

	filtered_sim_params_dataframe.to_csv(filtered_sim_params_name, index=False)
	print(f"Filtered hourly data saved to {filtered_sim_params_name}")
